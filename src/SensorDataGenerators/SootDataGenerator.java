...
public class SootDataGenerator implements SensorDataGenerator {
    ...
    public byte[] generateData(Integer x, Integer y, LocalTime time){
        ...
        if(x<210&&y< 230)
            return new byte[]{(byte)Math.floorMod((int) Math.round(97-10+(x+y)/250 +0.3*random.nextGaussian()),255)};
        else if(x<1100&&y< 1100)
            return new byte[]{(byte)Math.floorMod((int) Math.round(98-10+Math.log10((x+y)/50)+0.3*random.nextGaussian()),255)};
